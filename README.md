...
GPX uploads require a logged-in Netlify Identity user. New signups use the `Mannenavond` access key, but once a user is logged in they do not need to enter the key again to upload GPX routes. Netlify Functions verify the logged-in user before storing or deleting route files.

Required npm packages include `@netlify/blobs` and `@netlify/identity`.
